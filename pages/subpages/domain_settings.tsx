 ...
 const getMailboxes = useCallback(async () => {
  ...
  return (
    <>
        ...
        <DomainSettingsTopBar domain_id={domain._id} userInfo={userInfo} userDetails={userDetails} mailboxCount={mailboxes.length} rateLimit={rateLimit} refreshMailboxes={getMailboxes} domain={domain.domain} catch_all={domain.catch_all} />
        {mailboxes.length === 0 ? (
            ...
            {mailboxesToShow.map((mailbox: any, index: any) => (
              <MailboxCard key={index} domain_id={domain._id} id={mailbox.mailbox_id} fqe={mailbox.fqe} quota_used={mailbox.quota_used} quota={mailbox.quota} mails={mailbox.messages} active={mailbox.active} created={mailbox.created_at} />
            ))}
            </div>
